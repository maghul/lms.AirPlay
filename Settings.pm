# AirPlay for Linux Plugin for SqueezeCenter
...
package Plugins::AirPlay::Settings;
...
sub name {
        ...
        if ( substr( $::VERSION, 0, 3 ) lt 7.4 ) {
                return Slim::Web::HTTP::protectName('PLUGIN_AIRPLAY');
        }
        else {
                # $::noweb to detect TinySC or user with no web interface
                if ( !$::noweb ) {
                        return Slim::Web::HTTP::CSRF->protectName('PLUGIN_AIRPLAY');
                }
        }
...
sub page {
        ...
        # assumes at least SC 7.0
        if ( substr( $::VERSION, 0, 3 ) lt 7.4 ) {
                return Slim::Web::HTTP::protectURI('plugins/AirPlay/settings/basic.html');
        }
        else {
                # $::noweb to detect TinySC or user with no web interface
                if ( !$::noweb ) {
                        return Slim::Web::HTTP::CSRF->protectURI('plugins/AirPlay/settings/basic.html');
                }
        }
