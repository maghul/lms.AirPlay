...
sub _http_read {
        ...
        if ($code) {
                ...
                $log->debug("EMH: Falling through to _http_read_body...");
...
sub _http_read_body {
        ...
        #	my $mixed_bounday= $self->response->mixed_boundary;
        #	if ( $mixed_boundary ) {
        #		if ( $buf =~ /$mixed_boundary/ ) {
        #			main::DEBUGLOG && $log->debug("Mixed Boundary: Parsing chunk header." );
        #
        #		}
        #	}
        ...
        elsif ( $self->response->header('transfer-encoding') eq "chunked" ) {

                # Set the buffer as content in Response object
        ...
        $log->debug( "Body content: ", $self->response->content );
        ...
        if ( !defined $result || $result == 0 ) {
                ...
                # Some servers may never send EOF, but we want to return whatever data we've read
                my $timeout = $self->timeout || $prefs->get('remotestreamtimeout');
                $log->debug("Setting http_read_timeout =$timeout");
                Slim::Utils::Timers::setTimer( $socket, Time::HiRes::time() + $timeout, \&_http_read_timeout, $self, $args );
        }
}
